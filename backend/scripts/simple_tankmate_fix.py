...
class SimpleTankmateFixer:
    """Simple tankmate recommendations fixer with basic compatibility logic"""
    ...
    def _load_manual_fixes(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            # Additional problematic fish
            "angelfish": {
                "fully_compatible": [
                    "corydoras", "pleco", "tetra", "gourami", "danio"
                ],
                "conditional": [
                    "other angelfish", "dwarf cichlids", "barbs"
                ],
                "incompatible": [
                    "betta", "guppy", "tiger barb", "flowerhorn", "oscar"
                ],
                "conditions": {
                    "other angelfish": ["Large tank", "Monitor for aggression", "Provide territories"],
                    "dwarf cichlids": ["Large tank", "Monitor for aggression", "Provide hiding spots"]
                }
            },
            
            "tiger barb": {
                "fully_compatible": [
                    "other tiger barbs", "rosy barb", "tinfoil barb", "silver dollar"
                ],
                "conditional": [
                    "larger peaceful fish", "pleco", "corydoras"
                ],
                "incompatible": [
                    "betta", "angelfish", "guppy", "neon tetra", "small peaceful fish"
                ],
                "conditions": {
                    "larger peaceful fish": ["School of 6+ tiger barbs", "Large tank", "Monitor for fin nipping"],
                    "pleco": ["Large tank", "Monitor for aggression"]
                }
            }
        }
    ...
    def check_basic_compatibility(self, fish1: Dict, fish2: Dict) -> Tuple[str, List[str], List[str]]:
        """Basic compatibility check using simple logic"""
        fish1_name = fish1['common_name']
        fish2_name = fish2['common_name']
        
        # First check manual compatibility rules
        manual_result = self.check_manual_compatibility(fish1_name, fish2_name)
        if manual_result:
        ...
        # Check social behavior compatibility
        ...
        if 'solitary' in social1 and 'schooling' in social2:
            return ("conditional",
                   [f"{fish1_name} is solitary while {fish2_name} prefers schooling"],
                   ["Provide separate territories", "Monitor for stress", "Ensure adequate space"])
        
        if 'schooling' in social1 and 'solitary' in social2:
            return ("conditional",
                   [f"{fish1_name} prefers schooling while {fish2_name} is solitary"],
                   ["Provide separate territories", "Monitor for stress", "Ensure adequate space"])
    ...
    async def calculate_compatibility_matrix(self):
        ...
        logger.info("🧮 Calculating basic compatibility matrix...")
        ...
        for i, fish1 in enumerate(all_fish):
            for j, fish2 in enumerate(all_fish):
                ...
                try:
                    ...
                    compatibility_level, reasons, conditions = self.check_basic_compatibility(fish1, fish2)
                    ...
                    result = {
                        ...
                        'generation_method': 'basic_logic_with_manual_fixes',
    ...
    async def generate_improved_tankmate_recommendations(self):
        ...
        for fish_name, fish_data in self.fish_data.items():
            ...
            try:
                ...
                water_type = fish_data.get('water_type', '')
                
                # Determine special requirements
                special_requirements = []
                if water_type:
                    special_requirements.append(f"Water type: {water_type}")
                
                if care_level:
                    special_requirements.append(f"Care level: {care_level}")
                
                # Add specific requirements based on fish type
                fish_name_lower = fish_name.lower()
                if 'betta' in fish_name_lower:
                    special_requirements.extend([
                        "Requires 5+ gallon tank minimum",
                        "Prefers still or slow-moving water",
                        "Monitor for aggression with tankmates"
                    ])
                elif 'goldfish' in fish_name_lower:
                    special_requirements.extend([
                        "Cold water species (18-22°C)",
                        "Requires large tank (20+ gallons per fish)",
                        "Produces significant waste"
                    ])
                elif 'marine' in water_type.lower() or 'saltwater' in water_type.lower():
                    special_requirements.extend([
                        "Saltwater setup required",
                        "Proper salinity and pH maintenance",
                        "Marine-specific equipment needed"
                    ])
    ...
    async def save_to_files(self):
        ...
        try:
            ...
            summary = {
                ...
                'method': 'basic_logic_with_manual_fixes',
                'system_version': '2.1_simple'
            }
            
            summary_file = 'simple_compatibility_summary.json'
    ...
    async def fix_tankmate_recommendations(self):
        ...
        logger.info("🚀 Starting simple tankmate recommendations fix...")
        ...
        logger.info(f"🎉 Simple tankmate recommendations fix complete!")
...
async def main():
    """Fix tankmate recommendations using simple logic"""
    fixer = SimpleTankmateFixer()
